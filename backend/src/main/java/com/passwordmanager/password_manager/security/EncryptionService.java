...
public class EncryptionService {
  ...
  public SecretKey deriveKey(String password, byte[] salt) throws EncryptionException {
    try {
      ...
      log.error("Encryption Error deriving key");
      throw new EncryptionException(e.getMessage(), e.getCause());
    }
  }

  public EncryptedData encrypt(String plainText, SecretKey masterKey) throws EncryptionException {
    try {
      // Generate random IV
      ...
      log.error("Encryption error while encrypting");
      throw new EncryptionException(e.getMessage());
    }
  }

  public String decrypt(String encryptedText, String ivBase64, SecretKey masterKey) throws EncryptionException {
  try {
    ...
    return byteToString(decryptedBytes);
           ...
           InvalidAlgorithmParameterException | IllegalBlockSizeException | BadPaddingException e) {
    log.error("Decryption error while decrypting");
    throw new EncryptionException(e.getMessage());
  }
  }

  public String hashPassword(String password, byte[] salt) throws EncryptionException {
    try {
      SecretKeyFactory factory = SecretKeyFactory.getInstance(PDKDF_ALGORITHM);
      KeySpec spec = new PBEKeySpec(password.toCharArray(), salt, ITERATIONS, KEY_LENGTH);
      ...
      log.error("Encryption error while hashing");
      throw new EncryptionException(e.getMessage());
    }
  }

  public boolean matches(String password, String hashedPassword, byte[] salt) throws EncryptionException {
